...
Alert::Alert( PtgPlace* p, CString root, const char* jrServerName ) : Sock( jrServerName )
{
   ...
   usingJrServer = (strlen(jrServerName) == 0) ? UBQ_FALSE : UBQ_TRUE;
}
...
{
   if (usingJrServer && (doConnect() > -1)) {

	char* p = buff;
	UbqUlong  msgLen = sizeof(UbqUchar)   // type flag
...
Alert::broadcastToJr( Event* e )
{
   if (usingJrServer && (doConnect() > -1)) {
	...
	char* p = buff;
	UbqUlong msgLen = sizeof(UbqUchar) + sizeof(UbqUchar) + sizeof(UbqUchar);
	p += sizeof(msgLen);
	*p++ = e->getType();
	*p++ = 0;	// placeholder
	*p++ = 0;	// format: normal
