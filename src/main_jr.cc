//
// VP Event Notification Service - Junior version
//
// This is intended to work with the server for a secondary community,
// e.g. vpadult.com. It sends IMs to online users, but does not handle
// database access nor offline notifications.
//
...
int ubqAppMain(int argc, char* argv[])
{
  ...
  //
  UbqControlString c("VPALERT_ROOT", "/u/vplaces/VPCOM/VPALERTJR");
  ...
  // Process events
  //
  AlertJr alert(myPlace, root);
  int listenFd = alert.getListenFd();
  int width = 0;
  while (!gotDisconnected) {
    ...
    //
    // Messages from the main alert service are caught here.
    //
    if (r.isSet(listenFd)) {
      r.unset(listenFd);
      alert.doAccept();
